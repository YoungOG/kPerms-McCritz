...
public class SetUserSuffixCommand extends BaseCommand {

    @Command(name = "setusersuffix", aliases = {"setusuffix", "addusersuffix", "addusuffix"}, permission = "kperms.setusersuffix")
    public void onCommand(CommandArgs command) {
        CommandSender sender = command.getSender();
        String[] args = command.getArgs();

        if (args.length != 2) {
            MessageManager.sendMessage(sender, "&c/" + command.getLabel() + " <player> <suffix>");
            return;
        }
        ...
        getProfileManager().requestProfile(uuid, new ProfileRequest<Profile>() {
            ...
            public void onComplete(Profile result) {
                ...
                if (profile != null) {
                    profile.setSuffix(args[1]);
                    profile.updateTabListName();
                    profile.saveProfileData(true);

                    sender.sendMessage(ChatColor.GRAY + "The user suffix for " + ChatColor.RED + finalName + ChatColor.GRAY + " has been set to " + ChatColor.RED + args[1] + ChatColor.GRAY + ".");
                } else {
